...
function submitSearch() {
    ...
    fetchSearch(name, gender);
}

function fetchSearch(name, gender) {
    fetch(`/search/?name=${name}&gender=${gender}`)
    .then(response => {
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
      ...
      .then(data => {
        ...
        data.searchResults.forEach(user => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${user.Id}</td>
                <td>${user.Name}</td>
        ...
        if (data.page > 1) {
            const prevSpan = document.createElement('span');
            prevSpan.innerHTML = `<a href="/search/${data.page - 1}" onclick="fetchSearch('${name}', '${gender}')">Previous</a>`;
            paginationDiv.appendChild(prevSpan);
        }
        ...
        for (let num = startPage; num <= endPage; num++) {
            const pageSpan = document.createElement('span');
            pageSpan.innerHTML = `<a href="/search/${num}" onclick="fetchSearch('${name}', '${gender}')">${num}</a>`;
            paginationDiv.appendChild(pageSpan);
        }
        ...
        if (data.page < data.totalPages) {
            const nextSpan = document.createElement('span');
            nextSpan.innerHTML = `<a href="/search/${data.page + 1}" onclick="fetchSearch('${name}', '${gender}')">Next</a>`;
            paginationDiv.appendChild(nextSpan);
        }
    })
      .catch(error => {
